...
class userjournals_shortcodes extends e_shortcode
{
	...
	public function sc_uj_blogger_picture($parm = null)
	{
 
        $text = e107::getParser()->toAvatar($this->var, $parm);
		return $text;
	}
	...
	{
        if($parm === 'href') {
          return   e_PLUGIN_ABS . "userjournals/userjournals.php?blog." . $this->var["userjournals_id"];
        }
	...
	public function sc_uj_blog_categories_label($parm = '')
	{
		...
		if ($this->plugPrefs["userjournals_show_cats"] != 0 && strlen($this->var["userjournals_categories"]) > 0) {
			...
			for ($i=0; $i<count($userjournals_categories); $i++) {  
				...
				$text .=  $uj_category["userjournals_cat_name"];
