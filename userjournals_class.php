...
if (!class_exists("UserJournals")) {
   ...
   class UserJournals {
    ...
    public function render()
    {
			...
			if ($this->plugPrefs["userjournals_show_mood"] == 1)
			{
				...
				else
				{
					...
					foreach ($keys as $file => $key)
					{
						...
						$file = $this->mood[$key];
						$text .=
							"<input type='radio' name='journal_mood' value='$key' $checked> <img src='" . $jsmiles . $key . ".gif' alt= '{$file}' title='{$file}' style='margin-top: -30px;'>";
					}
     ...
     function latestjournals($limit = 10, $template = 'latest') {
     
        global $sc_style;
        $sc_style['UJ_BLOG_CATEGORIES_LABEL']['pre'] = "<span class='label label-default badge badge-secondary bg-secondary'>";
        $sc_style['UJ_BLOG_CATEGORIES_LABEL']['post'] = "</span>";
        $sc_style['UJ_BLOG_LINK']['pre'] = "";
        $sc_style['UJ_BLOG_LINK']['post'] = "";
		
        $template = e107::getTemplate('userjournals', 'userjournals_menu', 'latest', true, true);
 
        $start = $template['start'];
	    $end = $template['end'];
 
        $limit = " limit 0, 10 ";     //todo
        ...
        if ($results) {
			...
			$text = "";
			foreach($results AS $row) {
               $row['uj_categories'] = $this->cats; 
                
               $this->sc->setVars($row); 
               $text .= e107::getParser()->parseTemplate($template['item'], FALSE, $this->sc);
        ...
        e107::getRender()->tablerender($this->plugPrefs["userjournals_menu_title"], $start.$text.$end);
